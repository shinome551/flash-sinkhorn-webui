...
設計は [SPEC.md](SPEC.md)、実装タスクは [TODO.md](TODO.md) を参照。
...
モードの右の「ソフト / ハード」で対応の種類を切り替えられます (再実行不要)。ハードは c-transform の argmin による 1 件の対応で、
一様重みではソフトの top-1 と同じパッチを指します。違いは表示で、フローの矢印は分布の重心ではなく対応先の中心を指し、
カバレッジは「何個の A パッチに選ばれたか」(多対一の集中や、選ばれない B パッチ) になります。
...
仕様は SPEC 4 章、または <http://localhost:8000/docs> (OpenAPI)。

## 性能

SPEC 8 章の目標「512px / patch 16 (N = M = 768) で end-to-end 1 秒以内 (GPU)」の実測。
...
- `uv run python -m scripts.bench_ot`: OT コア単体のベンチ (結果は TODO.md の Phase 3)。flash では O(nd) 重心射影 (SPEC 3.5) の時間とチャンク要約との差も出す。
